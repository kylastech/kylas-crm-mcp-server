"""
shared/http_client.py — the HTTP layer every entity's Kylas API calls go
through: request/response logging, the 100–500 ms inter-call throttle, auth
header resolution (OAuth Bearer / x-api-key header / env fallback),
KylasAPIError, get_client(), and handle_api_response().

Imports `mcp`, `logger`, `BASE_URL`, `API_KEY`, `SERVER_VERSION` from
shared.app — never the other way around (app.py is the dependency root).
"""
...
from shared.app import mcp, logger, BASE_URL, API_KEY, SERVER_VERSION
