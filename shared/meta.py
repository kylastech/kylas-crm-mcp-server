"""
shared/meta.py — helpers and tools that are NOT specific to any one entity:
datetime conversion, entity-label fetching (logic only — the get_entity_labels
tool itself lives in main.py), search/field formatting (_format_field,
_build_search_json_rule and friends), payload normalization shared across
every entity's create/update, and the pipeline/product/user lookup tools.

Imports `mcp`, `logger`, `BASE_URL` from shared.app and `KylasAPIError`,
`get_client`, `handle_api_response`, `_reset_api_call_count` from
shared.http_client — never the other way around.
"""
...
# ---------------------------------------------------------------------------
# Entity labels (tenant-specific display names) — fetch logic only.
#
# get_entity_labels() (the actual @mcp.tool()) and entity_labels_resource()
# stay in main.py alongside get_current_user() — the "2 standalone tools"
# that don't fit the id-based registry flow.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Payload normalization shared by every entity's create/update
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Pipeline lookups (for pipeline + stage filters, and moving leads/deals
# through a pipeline)
# ---------------------------------------------------------------------------
...
async def _get_pipeline_details_raw(pipeline_id: int) -> Dict[str, Any]:
    """Fetch raw pipeline details dict from GET /pipelines/{id}. Used both by
    get_pipeline_details_logic below and by entities/deal.py's sequential-stage
    advancement (both need the same raw stages/reasons payload)."""
    async with get_client() as client:
...
# ---------------------------------------------------------------------------
# Product lookup (for the products filter on leads/deals)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# User lookup (for createdBy, updatedBy, ownerId, importedBy, convertedBy filters)
# ---------------------------------------------------------------------------
