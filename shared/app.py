"""
shared/app.py — env config, logging, SYSTEM_INSTRUCTIONS, lifespan, OAuth
wiring, and the `mcp` FastMCP instance.

This is the dependency root of the whole package: nothing it needs may
depend on anything that itself needs `mcp` back (http_client.py/meta.py
import `mcp` FROM here — never the other way around).
"""
